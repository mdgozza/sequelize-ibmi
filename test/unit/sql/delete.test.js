  ...
  describe('delete', () => {
    ...
    describe('truncate #4306', () => {
      ...
      it(util.inspect(options, { depth: 2 }), () => {
        return expectsql(sql.truncateTableQuery(options.table, options), {
    ...
    describe('delete with limit', () => {
      ...
      it(util.inspect(options, { depth: 2 }), () => {
        return expectsql(sql.deleteQuery(options.table, options.where, options, User), {
          ...
          sqlite:
            "DELETE FROM `public.test_users` WHERE rowid IN (SELECT rowid FROM `public.test_users` WHERE `name` = 'foo'';DROP TABLE mySchema.myTable;' LIMIT 10)",
          mssql:
            "DELETE TOP(10) FROM [public].[test_users] WHERE [name] = N'foo'';DROP TABLE mySchema.myTable;'; SELECT @@ROWCOUNT AS AFFECTEDROWS;",
    ...
    describe('delete with undefined parameter in where', () => {
      ...
      it(util.inspect(options, { depth: 2 }), () => {
        const sqlOrError = _.attempt(sql.deleteQuery.bind(sql), options.table, options.where, options, User);
