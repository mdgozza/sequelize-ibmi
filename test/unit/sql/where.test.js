    ...
    it("{ id: 1 }, { prefix: current.literal(sql.quoteTable.call(current.dialect.queryGenerator, {schema: 'yolo', tableName: 'User'})) }", () => {
      expectsql(
        sql.whereQuery(
          { id: 1 },
          {
            prefix: current.literal(
              sql.quoteTable.call(current.dialect.queryGenerator, {
                schema: 'yolo',
                tableName: 'User'
              })
            )
  ...
  describe('whereItemQuery', () => {
    ...
    describe('Op.in', () => {
      ...
      testsql(
        'equipment',
        {
          [Op.in]: current.literal('(select order_id from product_orders where product_id = 3)')
        },
        {
    ...
    describe('Op.notIn', () => {
      ...
      testsql(
        'equipment',
        {
          [Op.notIn]: current.literal('(select order_id from product_orders where product_id = 3)')
        },
        {
    ...
    describe('Op.and/Op.or/Op.not', () => {
      describe('Op.or', () => {
        ...
        testsql(Op.or, [{ email: 'maker@mhansen.io' }, { email: 'janzeh@gmail.com' }], {
          default: "([email] = 'maker@mhansen.io' OR [email] = 'janzeh@gmail.com')",
          mssql: "([email] = N'maker@mhansen.io' OR [email] = N'janzeh@gmail.com')"
        });
      ...
      describe('Op.and', () => {
        ...
        testsql(
          'name',
          {
            [Op.and]: [{ [Op.like]: '%someValue1%' }, { [Op.like]: '%someValue2%' }]
          },
          {
    ...
    describe('Op.col', () => {
      ...
      testsql(Op.or, [{ ownerId: { [Op.col]: 'user.id' } }, { ownerId: { [Op.col]: 'organization.id' } }], {
    ...
    if (current.dialect.supports.JSON) {
      describe('JSON', () => {
        ...
        it('sequelize.json({profile: {id: "12346-78912", name: "test"}})', function () {
          expectsql(
            sql.whereItemQuery(
              undefined,
              this.sequelize.json({
                profile: { id: '12346-78912', name: 'test' }
              })
            ),
            {
              ...
              mariadb:
                "json_unquote(json_extract(`profile`,'$.id')) = '12346-78912' AND json_unquote(json_extract(`profile`,'$.name')) = 'test'",
              mysql:
                "json_unquote(json_extract(`profile`,'$.\\\"id\\\"')) = '12346-78912' AND json_unquote(json_extract(`profile`,'$.\\\"name\\\"')) = 'test'"
            }
          );
        ...
        testsql(
          ...
          {
            mariadb:
              "(json_unquote(json_extract(`User`.`data`,'$.nested.attribute')) = 'value' AND json_unquote(json_extract(`User`.`data`,'$.nested.prop')) != 'None')",
            mysql:
              '(json_unquote(json_extract(`User`.`data`,\'$.\\"nested\\".\\"attribute\\"\')) = \'value\' AND json_unquote(json_extract(`User`.`data`,\'$.\\"nested\\".\\"prop\\"\')) != \'None\')',
            postgres:
        ...
        testsql(
          ...
          {
            mariadb:
              "(json_unquote(json_extract(`User`.`data`,'$.name.last')) = 'Simpson' AND json_unquote(json_extract(`User`.`data`,'$.employment')) != 'None')",
            mysql:
              "(json_unquote(json_extract(`User`.`data`,'$.\\\"name\\\".\\\"last\\\"')) = 'Simpson' AND json_unquote(json_extract(`User`.`data`,'$.\\\"employment\\\"')) != 'None')",
            postgres:
        ...
        testsql(
          ...
          {
            mariadb:
              "(CAST(json_unquote(json_extract(`data`,'$.price')) AS DECIMAL) = 5 AND json_unquote(json_extract(`data`,'$.name')) = 'Product')",
            mysql:
              "(CAST(json_unquote(json_extract(`data`,'$.\\\"price\\\"')) AS DECIMAL) = 5 AND json_unquote(json_extract(`data`,'$.\\\"name\\\"')) = 'Product')",
        ...
        testsql(
          ...
          },
          {
            ...
            mysql: `CAST(json_unquote(json_extract(\`data\`,'$.\\"nested\\".\\"attribute\\"')) AS DATETIME) > ${sql.escape(
              dt
            )}`,
