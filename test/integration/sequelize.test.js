...
describe(Support.getTestDialectTeaser('Sequelize'), () => {
  describe('constructor', () => {
    ...
    if (dialect === 'postgres') {
      const getConnectionUri = o =>
        `${o.protocol}://${o.username}:${o.password}@${o.host}${o.port ? `:${o.port}` : ''}/${o.database}`;
  ...
  describe('set', () => {
    it('should be configurable with global functions', function () {
      ...
      const testEntity = this.sequelize.define(
        'TestEntity',
        {},
        {
          setterMethods: {
  ...
  if (dialect === 'mysql') {
    describe('set', () => {
      it("should return an promised error if transaction isn't defined", async function () {
        await expect(this.sequelize.set({ foo: 'bar' })).to.be.rejectedWith(
          TypeError,
      ...
      it('one value', async function () {
        ...
        const data = await this.sequelize.query('SELECT @foo as `foo`', {
          plain: true,
          transaction: this.t
        });
      ...
      it('multiple values', async function () {
        ...
        const data = await this.sequelize.query('SELECT @foo as `foo`, @foos as `foos`', {
          plain: true,
          transaction: this.t
        });
        expect(data).to.be.ok;
  ...
  describe('sync', () => {
    it('synchronizes all models', async function () {
      ...
      const Task = this.sequelize.define(`task${config.rand()}`, {
        title: DataTypes.STRING
      });
    ...
    if (current.dialect.supports.transactions) {
      describe('transaction', () => {
        ...
        it('supports nested transactions using savepoints', async function () {
          const User = this.sequelizeWithTransaction.define('Users', {
            username: DataTypes.STRING
          });
        ...
        describe('supports rolling back to savepoints', () => {
          ...
          it('rolls back to the first savepoint, undoing everything', async function () {
            ...
            const sp1 = await this.sequelizeWithTransaction.transaction({
              transaction
            });
            this.sp1 = sp1;
            await this.User.create({}, { transaction: this.transaction });
            const sp2 = await this.sequelizeWithTransaction.transaction({
  ...
  describe('paranoid deletedAt non-null default value', () => {
    it('should use defaultValue of deletedAt in paranoid clause and restore', async function () {
      ...
      const fetchedDestroyedUser = await User.findByPk(destroyedUser.id, {
        paranoid: false
      });
