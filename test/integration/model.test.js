...
describe(Support.getTestDialectTeaser('Model'), () => {
  ...
  describe('update', () => {
    ...
    if (dialect === 'postgres') {
      it('returns the affected rows if `options.returning` is true', async function () {
        ...
        await this.User.bulkCreate(data);
        let [count, rows] = await this.User.update(
          { username: 'Bill' },
          {
        ...
        [count, rows] = await this.User.update(
          { username: 'Bill' },
          {
  ...
  describe('equals', () => {
    ...
    if (dialect !== 'sqlite') {
      it('correctly determines equality with multiple primary keys', async function () {
        ...
        const user = await userKeys.create({
          foo: '1',
          bar: '2',
          name: 'hallo',
          bio: 'welt'
  ...
  if (dialect !== 'sqlite') {
    describe('equalsOneOf', () => {
      ...
      it('determines equality if one is matching', async function () {
        const u = await this.userKey.create({
          foo: '1',
          bar: '2',
          name: 'hallo',
          bio: 'welt'
  ...
  describe('schematic support', () => {
    ...
    // this test was skipped in the original fork by way of a return statement.
    ...
    it.skip('should be able to reference a table with a schema set', async function () {
    ...
    it.skip('should be able to create and update records under any valid schematic', async function () {
      let logged = 0;
